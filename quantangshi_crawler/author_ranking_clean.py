...
class CleanAuthorRankingAnalyzer:
    ...
    def analyze_authors(self):
        ...
        for poem in self.poems_data:
            ...
            self.author_counts[clean_author] += 1
    ...
    def save_ranking_txt(self, output_file: str = "author_ranking_clean.txt"):
        """保存排名為文本格式"""
        print("💾 正在保存清理版作者排名...")
        ...
        print(f"✅ 清理版排名已保存到: {output_file}")
    
    def save_top_authors_clean(self, output_file: str = "top_100_authors_clean.txt", top_n: int = 100):
        """保存前N名作者 (清理版)"""
        print(f"💾 正在保存前{top_n}名作者 (清理版)...")
        
        ranking = self.get_author_ranking()[:top_n]
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"全唐詩前{top_n}名作者排名 (清理版)\n")
            f.write("=" * 40 + "\n")
            f.write(f"生成時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("注意: 已去掉作者名稱末尾的'著'字\n\n")
        ...
        print(f"✅ 前{top_n}名作者 (清理版) 已保存到: {output_file}")
...
def main():
    """主函數"""
    print("👥 全唐詩作者作品數量排序工具 (清理版)")
    print("=" * 40)
    
    analyzer = CleanAuthorRankingAnalyzer()
    ...
    analyzer.save_top_authors_clean("top_100_authors_clean.txt", 100)
    analyzer.save_top_authors_clean("top_50_authors_clean.txt", 50)
    analyzer.save_top_authors_clean("top_20_authors_clean.txt", 20)
    ...
    print("\n🎉 作者排名 (清理版) 完成！")
    print("📁 生成的文件:")
    print("   - author_ranking_clean.txt (完整排名)")
    print("   - top_100_authors_clean.txt (前100名)")
    print("   - top_50_authors_clean.txt (前50名)")
    print("   - top_20_authors_clean.txt (前20名)")
