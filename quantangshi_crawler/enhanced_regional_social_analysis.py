...
def analyze_regional_social_ngrams(ngram_file, df, ngram_type):
    ...
    for region in ngram_df['region'].unique():
        ...
        for social_class in region_data['social_class'].unique():
            ...
            # 獲取該地域-社會階層的所有詩人名單
            poets_in_class = class_data[['詩人', 'poem_count']].drop_duplicates()
            poets_in_class = poets_in_class.sort_values('poem_count', ascending=False)
            
            regional_social_stats[region][social_class] = {
                'total_freq': total_freq,
                'unique_ngrams': len(class_data),
                'top_50': top_50,
                'poets': poets_in_class
            }
    
    return regional_social_stats

def generate_enhanced_markdown_report(regional_social_stats_1gram, regional_social_stats_2gram, regional_social_stats_4gram, df):
    """生成包含詩人名單的增強版 Markdown 報告"""
    
    markdown_content = """# 地域與社會階層詩人用字特徵分析（完整版）
...
"""
    ...
    for region in sorted(df['region'].unique()):
        ...
        for social_class in sorted(region_data['social_class'].unique()):
            ...
            class_data = region_data[region_data['social_class'] == social_class]
            class_data = class_data.sort_values('poem_count', ascending=False)
            
            markdown_content += f"#### {social_class} ({len(class_data)}人)\n\n"
            
            # 顯示詩人名單
            markdown_content += "| 排名 | 詩人姓名 | 詩歌數量 | 性別 |\n"
            markdown_content += "|------|----------|----------|------|\n"
            
            for i, (_, poet) in enumerate(class_data.iterrows(), 1):
                gender = poet['性別'] if not pd.isna(poet['性別']) else '未知'
                markdown_content += f"| {i} | {poet['poet_name']} | {poet['poem_count']:,} | {gender} |\n"
    ...
    for region in sorted(regional_social_stats_1gram.keys()):
        ...
        for social_class in sorted(regional_social_stats_1gram[region].keys()):
            ...
            # 顯示詩人名單
            markdown_content += "**詩人名單**:\n"
            markdown_content += "| 排名 | 詩人姓名 | 詩歌數量 |\n"
            markdown_content += "|------|----------|----------|\n"
            
            for i, (_, poet) in enumerate(stats['poets'].iterrows(), 1):
...
def main():
    ...
    # 生成報告
    print("\n生成增強版 Markdown 報告...")
    markdown_content = generate_enhanced_markdown_report(
    ...
    for region in sorted(regional_social_stats_1gram.keys()):
        ...
        for social_class in sorted(regional_social_stats_1gram[region].keys()):
            stats = regional_social_stats_1gram[region][social_class]
            print(f"  {social_class}: 總詞頻 {stats['total_freq']:,}, 獨特單字 {stats['unique_ngrams']:,}, 詩人 {len(stats['poets'])} 人")
