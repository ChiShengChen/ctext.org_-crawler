...
def main():
    ...
    for i, row in df.iterrows():
        ...
        # 第一列：詩人名字和詩歌數量
        poet_info = str(values[0]) if pd.notna(values[0]) else ""
        poem_count = extract_poem_count(poet_info)
        
        # 提取詩人名字
        poet_name = "未知"
        if ':' in poet_info:
            poet_name = poet_info.split(':')[0].strip()
        elif '首' in poet_info:
            # 如果格式是 "XXX 首"，提取前面的部分
            parts = poet_info.split('首')
            if len(parts) > 1:
                poet_name = parts[0].strip()
        
        # 第二列：性別
        gender = None
        if pd.notna(values[1]) and str(values[1]).lower() in ['male', 'female']:
            gender = '男' if str(values[1]).lower() == 'male' else '女'
        
        # 第七列：地理信息
        region = "未知"
        if len(values) > 6 and pd.notna(values[6]):
            region = extract_region(values[6])
