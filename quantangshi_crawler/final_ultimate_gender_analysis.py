...
"""
最終終極修正版男女詩人詞頻分析
徹底修正所有問題，確保白居易、杜甫等著名詩人被正確識別和排名
"""
...
def analyze_gender_ngrams_final_ultimate(ngram_file, df, ngram_type):
    """最終終極版分析性別 n-gram 特徵"""
...
def ensure_merged_ngram_csv(ngram_dir: str, n: int) -> str:
    """確保存在 merged_{n}gram_詞頻統計.csv；若不存在則由作者級CSV合併產生。

    來源作者CSV檔名格式：{author}_{n}gram_詞頻統計.csv，欄位：['排名','詞彙','出現次數']
    目標合併檔欄位：['字詞','詩人','詞頻']
    """
    merged_path = os.path.join(ngram_dir, f"merged_{n}gram_詞頻統計.csv")
    if os.path.exists(merged_path):
        return merged_path
    pattern = os.path.join(ngram_dir, f"*_{n}gram_詞頻統計.csv")
    files = glob.glob(pattern)
    if not files:
        raise FileNotFoundError(f"找不到任何作者級 {n}-gram CSV，用於合併：{pattern}")
    rows = []
    for filepath in files:
        filename = os.path.basename(filepath)
        # 移除尾綴 _{n}gram_詞頻統計.csv 取得作者名
        suffix = f"_{n}gram_詞頻統計.csv"
        if filename.endswith(suffix):
            author = filename[: -len(suffix)]
        else:
            author = filename
        try:
            df_author = pd.read_csv(filepath)
        except Exception:
            continue
        # 期望欄位：排名, 詞彙, 出現次數
        if {'詞彙', '出現次數'}.issubset(set(df_author.columns)):
            for _, r in df_author.iterrows():
                token = str(r['詞彙'])
                try:
                    count = int(r['出現次數'])
                except Exception:
                    continue
                rows.append({'字詞': token, '詩人': author, '詞頻': count})
    if not rows:
        raise RuntimeError(f"作者級 {n}-gram CSV 內容為空，無法合併")
    merged_df = pd.DataFrame(rows, columns=['字詞', '詩人', '詞頻'])
    merged_df.to_csv(merged_path, index=False)
    return merged_path

def generate_final_ultimate_markdown_report(gender_stats_1gram, gender_stats_2gram, gender_stats_3gram, gender_stats_4gram, gender_stats_5gram, gender_stats_6gram, gender_stats_7gram, df):
    """生成最終終極版 Markdown 報告"""
    ...
    markdown_content = f"""# 男女詩人詞頻分析報告（最終終極修正版）
...
"""
    ...
    for gender in ['男', '女', '未知']:
        if gender in gender_stats_1gram:
            ...
            markdown_content += "\n"
            markdown_content += "**前500名單字**:\n"
            markdown_content += "| 排名 | 單字 | 詞頻 |\n"
            markdown_content += "|------|------|------|\n"
            
            for i, (_, row) in enumerate(stats['top_500'].iterrows(), 1):
    ...
    # 3-gram 分析
    markdown_content += "## 3-gram（三字詞）分析\n\n"
    for gender in ['男', '女', '未知']:
    ...
    # 5-gram 分析
    markdown_content += "## 5-gram（五字詞）分析\n\n"
    for gender in ['男', '女', '未知']:
        if gender in gender_stats_5gram:
            stats = gender_stats_5gram[gender]
            markdown_content += f"### {gender}性詩人\n\n"
            markdown_content += f"- **總詞頻**: {stats['total_freq']:,}\n"
            markdown_content += f"- **獨特五字詞數**: {stats['unique_ngrams']:,}\n"
            markdown_content += f"- **詩人數量**: {len(stats['poets'])}\n\n"
            ...
            markdown_content += "\n"
            markdown_content += "**前500名五字詞**:\n"
            markdown_content += "| 排名 | 五字詞 | 詞頻 |\n"
            markdown_content += "|------|--------|------|\n"
    ...
    # 6-gram 分析
    markdown_content += "## 6-gram（六字詞）分析\n\n"
    for gender in ['男', '女', '未知']:
        if gender in gender_stats_6gram:
            stats = gender_stats_6gram[gender]
            markdown_content += f"### {gender}性詩人\n\n"
            markdown_content += f"- **總詞頻**: {stats['total_freq']:,}\n"
            markdown_content += f"- **獨特六字詞數**: {stats['unique_ngrams']:,}\n"
            markdown_content += f"- **詩人數量**: {len(stats['poets'])}\n\n"
            ...
            markdown_content += "\n"
            markdown_content += "**前500名六字詞**:\n"
            markdown_content += "| 排名 | 六字詞 | 詞頻 |\n"
            markdown_content += "|------|--------|------|\n"
    ...
    # 7-gram 分析
    markdown_content += "## 7-gram（七字詞）分析\n\n"
    for gender in ['男', '女', '未知']:
        if gender in gender_stats_7gram:
            stats = gender_stats_7gram[gender]
            markdown_content += f"### {gender}性詩人\n\n"
            markdown_content += f"- **總詞頻**: {stats['total_freq']:,}\n"
            markdown_content += f"- **獨特七字詞數**: {stats['unique_ngrams']:,}\n"
            markdown_content += f"- **詩人數量**: {len(stats['poets'])}\n\n"
            ...
            markdown_content += "\n"
            markdown_content += "**前500名七字詞**:\n"
            markdown_content += "| 排名 | 七字詞 | 詞頻 |\n"
            markdown_content += "|------|--------|------|\n"
...
def main():
    ...
    # 分析 1-gram
    print("\n分析 1-gram 數據...")
    gender_stats_1gram = analyze_gender_ngrams_final_ultimate(
    ...
    # 分析 2-gram
    print("\n分析 2-gram 數據...")
    gender_stats_2gram = analyze_gender_ngrams_final_ultimate(
    ...
    # 分析 3-gram
    print("\n分析 3-gram 數據...")
    gender_stats_3gram = analyze_gender_ngrams_final_ultimate(
        '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/analysis_result/analysis_results_no_title/author_ngram_csvs/merged_3gram_詞頻統計.csv',
        df,
        '3-gram'
    )
    ...
    # 分析 4-gram
    print("\n分析 4-gram 數據...")
    gender_stats_4gram = analyze_gender_ngrams_final_ultimate(
    ...
    try:
        print("\n分析 5-gram 數據...")
        gender_stats_5gram = analyze_gender_ngrams_final_ultimate(
            '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/analysis_result/analysis_results_no_title/author_ngram_csvs/merged_5gram_詞頻統計.csv',
            df,
            '5-gram'
        )
    except Exception as e:
        print(f"5-gram 資料缺失或讀取失敗，已略過: {e}")
        gender_stats_5gram = {}
    
    # 分析 6-gram
    try:
        print("\n分析 6-gram 數據...")
        gender_stats_6gram = analyze_gender_ngrams_final_ultimate(
            '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/analysis_result/analysis_results_no_title/author_ngram_csvs/merged_6gram_詞頻統計.csv',
            df,
            '6-gram'
        )
    except Exception as e:
        print(f"6-gram 資料缺失或讀取失敗，已略過: {e}")
        gender_stats_6gram = {}
    
    # 分析 7-gram
    try:
        print("\n分析 7-gram 數據...")
        gender_stats_7gram = analyze_gender_ngrams_final_ultimate(
            '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/analysis_result/analysis_results_no_title/author_ngram_csvs/merged_7gram_詞頻統計.csv',
            df,
            '7-gram'
        )
    except Exception as e:
        print(f"7-gram 資料缺失或讀取失敗，已略過: {e}")
        gender_stats_7gram = {}
    
    # 生成報告
    print("\n生成最終終極版 Markdown 報告...")
    markdown_content = generate_final_ultimate_markdown_report(
        gender_stats_1gram,
        gender_stats_2gram,
        gender_stats_3gram,
        gender_stats_4gram,
    ...
    # 保存報告
    output_file = '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/gender_linguistic_analysis_report_final_ultimate.md'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(markdown_content)
    
    print(f"\n分析完成！最終終極版報告已保存至: {output_file}")
    
    # 顯示基本統計
    print("\n基本統計:")
    print("1-gram 分析性別數:", len(gender_stats_1gram))
    print("2-gram 分析性別數:", len(gender_stats_2gram))
    print("3-gram 分析性別數:", len(gender_stats_3gram))
    print("4-gram 分析性別數:", len(gender_stats_4gram))
    if gender_stats_5gram:
        print("5-gram 分析性別數:", len(gender_stats_5gram))
    if gender_stats_6gram:
        print("6-gram 分析性別數:", len(gender_stats_6gram))
    if gender_stats_7gram:
        print("7-gram 分析性別數:", len(gender_stats_7gram))
