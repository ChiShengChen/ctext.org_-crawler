...
def main():
    ...
    for i, row in df.iterrows():
        ...
        if pd.notna(values[0]):
            ...
            if '首' in first_col or any(char.isdigit() for char in first_col):
                poem_count = extract_poem_count(first_col)
                # 提取詩人名字
                if ':' in first_col:
                    poet_name = first_col.split(':')[0].strip()
                elif '首' in first_col:
                    # 如果格式是 "XXX 首"，提取前面的部分
                    parts = first_col.split('首')
                    if len(parts) > 1:
