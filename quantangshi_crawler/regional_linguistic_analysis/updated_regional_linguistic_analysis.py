...
# -*- coding: utf-8 -*-
"""
Updated Regional Linguistic Analysis for Tang Poetry
更新版地域語言分析：在圖表上顯示具體數字
"""
...
class UpdatedRegionalLinguisticAnalyzer:
    ...
    def calculate_regional_similarity(self, regional_matrix):
        ...
        normalized_matrix = normalized_matrix.replace([np.inf, -np.inf], 0)
        ...
        similarity_matrix = similarity_matrix.round(4)
    ...
    def plot_heatmap_with_numbers(self, data, title, filename, output_dir):
        """Plot a heatmap with numbers for similarity or frequency data."""
        ...
        # Use fmt='.3f' for better readability and ensure all cells show numbers
        ax = sns.heatmap(data_clean, annot=True, cmap='viridis', fmt=".3f", 
    ...
    def plot_clusters_with_numbers(self, cluster_results, title, filename, output_dir):
        """Plot clustering results with numbers."""
        plt.figure(figsize=(12, 10))
        
        # Create scatter plot
        sns.scatterplot(data=cluster_results, x='pca1', y='pca2', hue='cluster', 
                        palette='deep', s=150, alpha=0.8)
        
        # Add region labels with numbers
        for i, row in cluster_results.iterrows():
            plt.text(row['pca1'] + 0.02, row['pca2'] + 0.02, f"{row['region']}\n({row['cluster']})", 
    ...
    def save_results(self, results, output_dir):
        ...
        with open(os.path.join(output_dir, 'updated_analysis_results.json'), 'w', encoding='utf-8') as f:
...
def main():
    ...
    analyzer = UpdatedRegionalLinguisticAnalyzer(poet_geo_path, ngram_dir)
    print("=== Updated Regional Linguistic Analysis ===")
    ...
    for ngram_type in ['1gram', '2gram', '4gram']:
        ...
        if regional_matrix is not None and not regional_matrix.empty:
            # Plot heatmap of regional n-gram frequencies with numbers
            analyzer.plot_heatmap_with_numbers(regional_matrix, 
                                f'Top 50 {ngram_type.capitalize()} Frequencies by Region (with Numbers)',
                                f'updated_regional_{ngram_type}_heatmap.png', output_dir)
            
            # Calculate and plot similarity matrix with numbers
            similarity_matrix = analyzer.calculate_regional_similarity(regional_matrix)
            analyzer.plot_heatmap_with_numbers(similarity_matrix, 
                                f'Regional {ngram_type.capitalize()} Cosine Similarity (with Numbers)',
                                f'updated_regional_{ngram_type}_similarity.png', output_dir)
            
            # Perform and plot clustering with numbers
            ...
            if n_clusters > 1:
                ...
                analyzer.plot_clusters_with_numbers(cluster_results, 
                                    f'Regional {ngram_type.capitalize()} Clustering (PCA) with Numbers',
                                    f'updated_regional_{ngram_type}_clusters.png', output_dir)
    ...
    print(f"\nUpdated regional linguistic analysis completed!")
    print(f"Found {len(matched_poets)} matched poets between datasets")
    print("All charts now include detailed numbers!")
