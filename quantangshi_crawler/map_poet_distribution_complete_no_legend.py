...
# -*- coding: utf-8 -*-
"""
Tang Poetry Poet Geographical Distribution Map - Complete Version (No Legend)
包含所有可识别地理信息的诗人（扩展识别规则）- 无图例版本
"""
...
def create_complete_bubble_map_no_legend(df):
    """Create complete bubble map without legend - full width"""
    print("\nCreating complete bubble map (no legend version)...")
    ...
    # Setup - full width without legend space
    plt.rcParams['font.family'] = 'DejaVu Sans'
    fig = plt.figure(figsize=(26, 20), dpi=150)
    # Use full width: changed from [0.05, 0.05, 0.72, 0.90] to [0.05, 0.05, 0.92, 0.90]
    ax = fig.add_axes([0.05, 0.05, 0.92, 0.90])
    ...
    # No legend axes added here - that's the main change!
...
def main():
    """Main function"""
    print("=" * 90)
    print("Tang Dynasty Poet Distribution - Complete Version (No Legend)")
    print("=" * 90)
    
    df = load_poet_geo_data_extended()
    print(f"\n✅ Loaded {len(df)} poets from database")
    
    fig = create_complete_bubble_map_no_legend(df)
    
    output_file = '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/tang_poet_map_complete_no_legend.png'
    fig.savefig(output_file, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"\n✅ Complete map (no legend) saved: {output_file}")
    
    output_hd = '/media/meow/One Touch/ctext.org_-crawler/quantangshi_crawler/tang_poet_map_complete_no_legend_HD.png'
    fig.savefig(output_hd, dpi=600, bbox_inches='tight', facecolor='white')
    print(f"✅ HD version (no legend) saved: {output_hd}")
    
    plt.close()
    print("\n" + "=" * 90)
    print("✨ Complete map (no legend) finished!")
    print("=" * 90)
