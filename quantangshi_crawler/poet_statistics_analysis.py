...
"""
    ...
    top_poets = df.nlargest(20, 'poem_count')[['詩人', 'gender_clean', 'region', 'poem_count']]
    for i, (_, row) in enumerate(top_poets.iterrows(), 1):
        poet_name = str(row['詩人']).split(':')[0].strip() if ':' in str(row['詩人']) else str(row['詩人'])
